...
async function main() {
	...
	for (const file of heroDesktop) {
		await convertImage(
			path.join(heroDesktopDir, file),
			`static/optimized/${path.parse(file).name}.webp`,
			1920,
			80
		);
	}
	...
	const totalOptimized = fs
		.readdirSync('static/optimized')
		.reduce((sum, f) => sum + fs.statSync(path.join('static/optimized', f)).size, 0);
